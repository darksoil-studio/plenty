...
export class CreateProducerDelivery extends SignalWatcher(LitElement) {
  ...
  async createProducerDelivery(fields: Partial<ProducerDelivery>) {
    if (this.orderHash === undefined)
      throw new Error(
        "Cannot create a new Producer Delivery without its order_hash field",
      );
    if (this.producerHash === undefined)
      throw new Error(
        "Cannot create a new Producer Delivery without its producer_hash field",
      );
    if (this.products === undefined)
      throw new Error(
        "Cannot create a new Producer Delivery without its products field",
      );
    ...
    try {
      ...
      const record: EntryRecord<ProducerDelivery> =
        await this.ordersStore.client.createProducerDelivery(producerDelivery);
