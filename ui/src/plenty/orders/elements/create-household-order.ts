...
export class CreateHouseholdOrder extends SignalWatcher(LitElement) {
  ...
  async createHouseholdOrder(fields: Partial<HouseholdOrder>) {
    if (this.orderHash === undefined) throw new Error('Cannot create a new Household Order without its order_hash field');
    if (this.products === undefined) throw new Error('Cannot create a new Household Order without its products field');
    ...
    try {
      ...
      const record: EntryRecord<HouseholdOrder> = await this.ordersStore.client.createHouseholdOrder(householdOrder);

      this.dispatchEvent(new CustomEvent('household-order-created', {
        composed: true,
        bubbles: true,
        detail: {
