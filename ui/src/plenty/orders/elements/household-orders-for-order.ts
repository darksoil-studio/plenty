...
export class HouseholdOrdersForOrder extends SignalWatcher(LitElement) {
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) 
      return html` <div class="column center-content" style="gap: 16px;">
        ...
        ></sl-icon>
        <span class="placeholder">${msg("No household orders found for this order")}</span>
      </div>`;

    return html`
      <div style="display: flex; flex-direction: column">
        ${hashes.map(hash =>
          html`<household-order-summary .householdOrderHash=${hash}></household-order-summary>`
        )}
      </div>
    `;
  }
  ...
  render() {
    const map = this.ordersStore.orders.get(this.orderHash).householdOrders.live.get();
