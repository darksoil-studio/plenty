...
export class EditProducerDelivery extends SignalWatcher(LitElement) {
  ...
  async firstUpdated() {
    const currentRecord = await toPromise(this.ordersStore.producerDeliveries.get(this.producerDeliveryHash).latestVersion);
    setTimeout(() => {
      (this.shadowRoot?.getElementById('form') as HTMLFormElement).reset();
    });
  }

  async updateProducerDelivery(currentRecord: EntryRecord<ProducerDelivery>, fields: Partial<ProducerDelivery>) {  
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('producer-delivery-updated', {
        composed: true,
        bubbles: true,
        detail: {
    ...
    return html`
      <sl-card>
        ...
        <form
          ...
          ${onSubmit(fields => this.updateProducerDelivery(currentRecord, fields))}
        >  
