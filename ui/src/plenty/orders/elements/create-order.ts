...
export class CreateOrder extends SignalWatcher(LitElement) {
  /**
   * REQUIRED. The status for this Order
   */
  @property()
  status!: OrderStatus;
  ...
  async createOrder(fields: Partial<Order>) {
    if (this.status === undefined) throw new Error('Cannot create a new Order without its status field');
    ...
    try {
      ...
      const record: EntryRecord<Order> = await this.ordersStore.client.createOrder(order);
