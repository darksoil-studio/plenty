...
export class CreateProducerInvoice extends SignalWatcher(LitElement) {
  ...
  /**
   * REQUIRED. The producer hash for this ProducerInvoice
   */
  ...
  async createProducerInvoice(fields: Partial<ProducerInvoice>) {
    if (this.orderHash === undefined)
      throw new Error(
        "Cannot create a new Producer Invoice without its order_hash field",
      );
    ...
    try {
      ...
      const record: EntryRecord<ProducerInvoice> =
        await this.ordersStore.client.createProducerInvoice(producerInvoice);

      this.dispatchEvent(
        new CustomEvent("producer-invoice-created", {
          composed: true,
          bubbles: true,
          detail: {
