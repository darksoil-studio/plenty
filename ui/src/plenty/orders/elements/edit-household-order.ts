...
export class EditHouseholdOrder extends SignalWatcher(LitElement) {
  ...
  async firstUpdated() {
    const currentRecord = await toPromise(this.ordersStore.householdOrders.get(this.householdOrderHash).latestVersion);
    setTimeout(() => {
      (this.shadowRoot?.getElementById('form') as HTMLFormElement).reset();
    });
  }

  async updateHouseholdOrder(currentRecord: EntryRecord<HouseholdOrder>, fields: Partial<HouseholdOrder>) {  
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('household-order-updated', {
        composed: true,
        bubbles: true,
        detail: {
    ...
    return html`
      <sl-card>
        ...
        <form
          ...
          ${onSubmit(fields => this.updateHouseholdOrder(currentRecord, fields))}
        >  
