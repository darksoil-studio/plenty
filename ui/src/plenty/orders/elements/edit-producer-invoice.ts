...
export class EditProducerInvoice extends SignalWatcher(LitElement) {
  ...
  async firstUpdated() {
    const currentRecord = await toPromise(
      this.ordersStore.producerInvoices.get(this.producerInvoiceHash)
        .latestVersion,
    );
    setTimeout(() => {
  ...
  async updateProducerInvoice(
    currentRecord: EntryRecord<ProducerInvoice>,
    fields: Partial<ProducerInvoice>,
  ) {
    ...
    try {
      ...
      this.dispatchEvent(
        new CustomEvent("producer-invoice-updated", {
          composed: true,
          bubbles: true,
          detail: {
  ...
  renderEditForm(currentRecord: EntryRecord<ProducerInvoice>) {
    return html` <sl-card>
      ...
      <form
        ...
        ${onSubmit((fields) =>
          this.updateProducerInvoice(currentRecord, fields),
        )}
      >
        <upload-files
          name="invoice"
          ...
          required
          .defaultValue=${currentRecord.entry.invoice}
        ></upload-files>
  ...
  render() {
    const producerInvoice = this.ordersStore.producerInvoices
      .get(this.producerInvoiceHash)
      .latestVersion.get();
