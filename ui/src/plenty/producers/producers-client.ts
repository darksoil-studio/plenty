...
export class ProducersClient extends ZomeClient<ProducersSignal> {
  ...
  async getLatestProduct(productHash: ActionHash): Promise<EntryRecord<Product> | undefined> {
    const record: Record = await this.callZome('get_latest_product', productHash);
    return record ? new EntryRecord(record) : undefined;
  }

  async getOriginalProduct(productHash: ActionHash): Promise<EntryRecord<Product> | undefined> {
    const record: Record = await this.callZome('get_original_product', productHash);
    return record ? new EntryRecord(record) : undefined;
  }

  async getAllRevisionsForProduct(productHash: ActionHash): Promise<Array<EntryRecord<Product>>> {
    const records: Record[] = await this.callZome('get_all_revisions_for_product', productHash);
    return records.map(r => new EntryRecord(r));
  }

  async updateProduct(originalProductHash: ActionHash, previousProductHash: ActionHash, updatedProduct: Product): Promise<EntryRecord<Product>> {
  ...
  getAllDeletesForProduct(originalProductHash: ActionHash): Promise<Array<SignedActionHashed<Delete>>> {
    return this.callZome('get_all_deletes_for_product', originalProductHash);
  }

  getOldestDeleteForProduct(originalProductHash: ActionHash): Promise<SignedActionHashed<Delete> | undefined> {
  ...
  async getDeletedProductsForProducer(producerHash: ActionHash): Promise<Array<[SignedActionHashed<CreateLink>, SignedActionHashed<DeleteLink>[]]>> {
