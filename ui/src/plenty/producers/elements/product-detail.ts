  ...
  renderDetail(entryRecord: EntryRecord<Product>) {
    return html`
      <sl-card>
          ...
          <sl-icon-button .src=${wrapPathInSvg(mdiDelete)} @click=${() => this.deleteProduct()}></sl-icon-button>
        </div>
        ...
        <div class="column" style="gap: 16px;">
          ...
          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Product Id")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.product_id }</span>
	  </div>
          ...
          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Categories")}</strong></span>
              ${ entryRecord.entry.categories.map(el => html`<span style="white-space: pre-line">${ el }</span>`)}	  </div>

          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Packaging")}</strong></span>
 	        <span style="white-space: pre-line">${  entryRecord.entry.packaging.type === 'Piece' ? "Piece" :  entryRecord.entry.packaging.type === 'Kilograms' ? "Kilograms" :  entryRecord.entry.packaging.type === 'Grams' ? "Grams" :  entryRecord.entry.packaging.type === 'Liters' ? "Liters" :  entryRecord.entry.packaging.type === 'Pounds' ? "Pounds" :  "Ounces"  }</span>
	  </div>

          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Maximum Available")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.maximum_available }</span>
	  </div>
          ...
          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Vat Percentage")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.vat_percentage }</span>
	  </div>

          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Margin Percentage")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.margin_percentage }</span>
	  </div>
          ...
          <div class="column" style="gap: 8px;">
	        <span><strong>${msg("Ingredients")}</strong></span>
 	        <span style="white-space: pre-line">${ entryRecord.entry.ingredients }</span>
	  </div>
      ...
      case 'completed':
        if (this._editing) {
      	  return html`<edit-product
      	    .productHash=${this.productHash}
            @product-updated=${async () => { this._editing = false; } }
