...
export class CreateProduct extends SignalWatcher(LitElement) {
  ...
  async createProduct(fields: Partial<Product>) {
    if (this.producerHash === undefined) throw new Error('Cannot create a new Product without its producer_hash field');
    ...
    try {
      ...
      const record: EntryRecord<Product> = await this.producersStore.client.createProduct(product);
    ...
    return html`
      <sl-card style="flex: 1;">
        ...
        >  
          ...
          <div>
          <sl-input name="product_id" .label=${msg("Product Id")}  required></sl-input>          </div>
          ...
          <div class="column" style="gap: 8px">
            ...
            ${repeat(this._categoriesFields, i => i, index => html`<div class="row" style="align-items: center;"><sl-input name="categories" .label=${msg("")} ></sl-input> <sl-icon-button .src=${wrapPathInSvg(mdiDelete)} @click=${() => { this._categoriesFields = this._categoriesFields.filter(i => i !== index) } }></sl-icon-button></div>`)}
  ...
  <sl-option value="Ounces">Ounces</sl-option>
          ...
          <div>
          <sl-input type="number" name="maximum_available" .label=${msg("Maximum Available")} ></sl-input>          </div>

          <div>
          <sl-input type="number" name="price" .label=${msg("Price")}  required></sl-input>          </div>

          <div>
          <sl-input type="number" name="vat_percentage" .label=${msg("Vat Percentage")}  required></sl-input>          </div>

          <div>
          <sl-input type="number" name="margin_percentage" .label=${msg("Margin Percentage")} ></sl-input>          </div>

          <div>
          <sl-input name="origin" .label=${msg("Origin")} ></sl-input>          </div>

          <div>
          <sl-textarea name="ingredients" .label=${msg("Ingredients")} ></sl-textarea>          </div>
