...
export class HouseholdsZomeMock extends ZomeMock implements AppAgentClient {
  ...
  householdMembershipClaims = new HoloHashMap<ActionHash, {
    deletes: Array<SignedActionHashed<Delete>>;
    revisions: Array<Record>;
  }>();

  async create_household_membership_claim(householdMembershipClaim: HouseholdMembershipClaim): Promise<Record> {
    const entryHash = hash(householdMembershipClaim, HashType.ENTRY);
    const record = await fakeRecord(await fakeCreateAction(entryHash), fakeEntry(householdMembershipClaim));
  ...
  async get_household_membership_claim(householdMembershipClaimHash: ActionHash): Promise<Record | undefined> {
    const householdMembershipClaim = this.householdMembershipClaims.get(householdMembershipClaimHash);
    return householdMembershipClaim ? householdMembershipClaim.revisions[0] : undefined;
  }

  async get_active_households(_: any): Promise<Array<Link>> {
    const records: Record[] = Array.from(this.households.values()).map(r => r.revisions[r.revisions.length - 1]);
...
export async function sampleHousehold(client: HouseholdsClient, partialHousehold: Partial<Household> = {}): Promise<Household> {
  return {
    ...{
      name: "Lorem ipsum 2",
...
export async function sampleHouseholdMembershipClaim(client: HouseholdsClient, partialHouseholdMembershipClaim: Partial<HouseholdMembershipClaim> = {}): Promise<HouseholdMembershipClaim> {
  return {
    ...{
