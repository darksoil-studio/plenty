...
test('create and read Household', async () => {
  await runScenario(async scenario => {
    ...
    const createReadOutput: EntryRecord<Household> = await toPromise(bob.store.households.get(household.actionHash).original);
...
test('create and update Household', async () => {
  await runScenario(async scenario => {
    ...
    let updatedHousehold: EntryRecord<Household> = await alice.store.client.updateHousehold(originalActionHash, originalActionHash, contentUpdate);
    ...
    const readUpdatedOutput0: EntryRecord<Household> = await toPromise(bob.store.households.get(household.actionHash).latestVersion);
    ...
    updatedHousehold = await alice.store.client.updateHousehold(originalActionHash, updatedHousehold.actionHash, contentUpdate);
    assert.ok(updatedHousehold);
    ...
    // Bob gets the updated Household
    const readUpdatedOutput1: EntryRecord<Household> = await toPromise(bob.store.households.get(originalActionHash).latestVersion);
...
test('create and delete Household', async () => {
  await runScenario(async scenario => {
    ...
    const deleteActionHash = await alice.store.client.deleteHousehold(household.actionHash);
    ...
    const deletes: Array<SignedActionHashed<Delete>> = await toPromise(bob.store.households.get(household.actionHash).deletes);
