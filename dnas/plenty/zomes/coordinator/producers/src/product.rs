        ...
        pub fn create_product(product: Product) -> ExternResult<Record> {
            ...
            create_link(product.producer_hash.clone(), product_hash.clone(), LinkTypes::ProducerToProducts, ())?;
            
        
            let record = get(product_hash.clone(), GetOptions::default())?
                .ok_or(wasm_error!(WasmErrorInner::Guest("Could not find the newly created Product".to_string())))?;
            Ok(record)
        }
        ...
        pub fn get_latest_product(original_product_hash: ActionHash) -> ExternResult<Option<Record>> {
            let links = get_links(
                GetLinksInputBuilder::try_new(original_product_hash.clone(), LinkTypes::ProductUpdates)?.build(),
            )?;
        ...
        #[hdk_extern]
        pub fn get_all_revisions_for_product(original_product_hash: ActionHash) -> ExternResult<Vec<Record>> {
    ...
    pub fn update_product(input: UpdateProductInput) -> ExternResult<Record> {
        let updated_product_hash = update_entry(input.previous_product_hash.clone(), &input.updated_product)?;
                
        create_link(input.original_product_hash.clone(), updated_product_hash.clone(), LinkTypes::ProductUpdates, ())?;

        let record = get(updated_product_hash.clone(), GetOptions::default())?
                .ok_or(wasm_error!(WasmErrorInner::Guest("Could not find the newly updated Product".to_string())))?;
        ...
        pub fn delete_product(original_product_hash: ActionHash) -> ExternResult<ActionHash> {
                ...
                let details = get_details(original_product_hash.clone(), GetOptions::default())?
                ...
                let entry = record.entry().as_option().ok_or(wasm_error!(WasmErrorInner::Guest("Product record has no entry".to_string())))?;
                let product = Product::try_from(entry)?;
                
                        let links = get_links(
                            GetLinksInputBuilder::try_new(product.producer_hash.clone(), LinkTypes::ProducerToProducts)?.build(),
                        )?;
                        for link in links {
        ...
        pub fn get_products_for_producer(producer_hash: ActionHash) -> ExternResult<Vec<Link>> {
            get_links(
                GetLinksInputBuilder::try_new(producer_hash, LinkTypes::ProducerToProducts)?.build(),
            )
        }
