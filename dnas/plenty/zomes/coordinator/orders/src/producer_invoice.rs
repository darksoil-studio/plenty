...
#[hdk_extern]
pub fn create_producer_invoice(producer_invoice: ProducerInvoice) -> ExternResult<Record> {
    let producer_invoice_hash =
        create_entry(&EntryTypes::ProducerInvoice(producer_invoice.clone()))?;
    create_link(
    ...
    create_link(
        producer_invoice.producer_hash.clone(),
        producer_invoice_hash.clone(),
        LinkTypes::ProducerToProducerInvoices,
        (),
    )?;
    let record = get(producer_invoice_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
        WasmErrorInner::Guest("Could not find the newly created ProducerInvoice".to_string())
    ))?;
    Ok(record)
}
...
pub fn get_original_producer_invoice(
    ...
    let Some(details) = get_details(original_producer_invoice_hash, GetOptions::default())? else {
        return Ok(None);
    };
    match details {
...
pub fn get_all_revisions_for_producer_invoice(
    ...
    let Some(Details::Record(details)) =
        get_details(original_producer_invoice_hash, GetOptions::default())?
    else {
        return Ok(vec![]);
    };
    ...
    for update in details.updates {
        let mut update_records =
            get_all_revisions_for_producer_invoice(update.action_address().clone())?;
...
#[hdk_extern]
pub fn update_producer_invoice(input: UpdateProducerInvoiceInput) -> ExternResult<Record> {
    let updated_producer_invoice_hash = update_entry(
        input.previous_producer_invoice_hash,
        &input.updated_producer_invoice,
    )?;
    let record =
        get(updated_producer_invoice_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
            WasmErrorInner::Guest("Could not find the newly updated ProducerInvoice".to_string())
        ))?;
    Ok(record)
}
...
pub fn delete_producer_invoice(
    ...
    .ok_or(wasm_error!(WasmErrorInner::Guest(
        "ProducerInvoice not found".to_string()
    )))?;
    ...
    let entry = record
        ...
        .as_option()
        .ok_or(wasm_error!(WasmErrorInner::Guest(
            "ProducerInvoice record has no entry".to_string()
        )))?;
...
pub fn get_oldest_delete_for_producer_invoice(
    ...
    let Some(mut deletes) = get_all_deletes_for_producer_invoice(original_producer_invoice_hash)?
    else {
        return Ok(None);
    };
...
#[hdk_extern]
pub fn get_producer_invoices_for_order(order_hash: ActionHash) -> ExternResult<Vec<Link>> {
    get_links(
        GetLinksInputBuilder::try_new(order_hash, LinkTypes::OrderToProducerInvoices)?.build(),
    )
}
