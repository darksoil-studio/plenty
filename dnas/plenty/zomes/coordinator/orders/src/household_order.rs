...
pub fn create_household_order(household_order: HouseholdOrder) -> ExternResult<Record> {
    let household_order_hash = create_entry(
        &EntryTypes::HouseholdOrder(household_order.clone()),
    )?;
    create_link(
    ...
    let record = get(household_order_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Could not find the newly created HouseholdOrder"
                .to_string())
            ),
        )?;
    Ok(record)
...
pub fn get_all_revisions_for_household_order(
    ...
    let Some(original_record) = get_original_household_order(
        original_household_order_hash.clone(),
    )? else {
        return Ok(vec![]);
    };
...
pub fn update_household_order(input: UpdateHouseholdOrderInput) -> ExternResult<Record> {
    ...
    let record = get(updated_household_order_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("Could not find the newly updated HouseholdOrder"
                .to_string())
            ),
        )?;
    Ok(record)
...
pub fn delete_household_order(
    ...
    let details = get_details(
        ...
        .ok_or(
            wasm_error!(WasmErrorInner::Guest("HouseholdOrder not found".to_string())),
        )?;
    ...
    let entry = record
        ...
        .ok_or(
            wasm_error!(
                WasmErrorInner::Guest("HouseholdOrder record has no entry".to_string())
            ),
        )?;
...
pub fn get_oldest_delete_for_household_order(
    ...
    let Some(mut deletes) = get_all_deletes_for_household_order(
        original_household_order_hash,
    )? else {
        return Ok(None);
    };
...
#[hdk_extern]
pub fn get_household_orders_for_order(
    order_hash: ActionHash,
) -> ExternResult<Vec<Link>> {
    get_links(
        GetLinksInputBuilder::try_new(order_hash, LinkTypes::OrderToHouseholdOrders)?
            .build(),
    )
}
