...
pub async fn sample_household_membership_claim_1(conductor: &SweetConductor, zome: &SweetZome) -> HouseholdMembershipClaim {
...
pub async fn sample_household_membership_claim_2(conductor: &SweetConductor, zome: &SweetZome) -> HouseholdMembershipClaim {
    HouseholdMembershipClaim {
	  member_create_link_hash: ::fixt::fixt!(ActionHash),
	  household_hash: ::fixt::fixt!(ActionHash),
    }
...
pub async fn create_household_membership_claim(conductor: &SweetConductor, zome: &SweetZome, household_membership_claim: HouseholdMembershipClaim) -> Record {
    let record: Record = conductor
        .call(zome, "create_household_membership_claim", household_membership_claim)
        .await;
    record
}
