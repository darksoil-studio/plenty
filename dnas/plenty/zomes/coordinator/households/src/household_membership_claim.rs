...
pub fn create_household_membership_claim(
    ...
    let household_membership_claim_hash = create_entry(&EntryTypes::HouseholdMembershipClaim(
        household_membership_claim.clone(),
    ))?;
    let record = get(
        household_membership_claim_hash.clone(),
        GetOptions::default(),
    )?
    .ok_or(wasm_error!(WasmErrorInner::Guest(String::from(
        "Could not find the newly created HouseholdMembershipClaim"
    ))))?;
    Ok(record)
}

#[hdk_extern]
pub fn query_my_household_membership_claims() -> ExternResult<Vec<Record>> {
...
pub fn get_household_membership_claim(
    ...
    let Some(details) = get_details(household_membership_claim_hash, GetOptions::default())? else {
        return Ok(None);
    };
    match details {
