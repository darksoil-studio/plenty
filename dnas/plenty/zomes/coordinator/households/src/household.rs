...
pub fn create_household(household: Household) -> ExternResult<Record> {
    ...
    let record = get(household_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
...
#[hdk_extern]
pub fn get_original_household(
    original_household_hash: ActionHash,
) -> ExternResult<Option<Record>> {
    let Some(details) = get_details(original_household_hash, GetOptions::default())?
    else {
        return Ok(None);
    };
    match details {
...
pub fn get_all_revisions_for_household(
    ...
    let Some(original_record) = get_original_household(original_household_hash.clone())?
    else {
        return Ok(vec![]);
    };
    let links = get_links(
...
pub fn update_household(input: UpdateHouseholdInput) -> ExternResult<Record> {
    ...
    let record = get(updated_household_hash.clone(), GetOptions::default())?
        .ok_or(
            wasm_error!(
...
#[hdk_extern]
pub fn delete_household(
    original_household_hash: ActionHash,
) -> ExternResult<ActionHash> {
    let details = get_details(original_household_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Household not found"))))?;
