...
use household::household_to_members::{RemoveMemberForHouseholdInput, AddMemberForHouseholdInput };
...
async fn link_a_household_to_a_member() {
    ...
    let links_output: Vec<Link> = conductors[1]
        .call(&bob_zome, "get_members_for_household", base_address.clone()).await;
    assert_eq!(links_output.len(), 0);

    // Alice creates a link from Household to Member
    let _result: () = conductors[0]
        .call(&alice_zome, "add_member_for_household", AddMemberForHouseholdInput {
        base_household_hash: base_address.clone(),
        target_member: target_address.clone()
     }).await;
    ...
    let _result: () = conductors[0]
        .call(&alice_zome, "remove_member_for_household", RemoveMemberForHouseholdInput {
        base_household_hash: base_address.clone(),
        target_member: target_address.clone()
     }).await;
    ...
    let deleted_links_output: Vec<(SignedActionHashed, Vec<SignedActionHashed>)> = conductors[1]
        .call(&bob_zome, "get_deleted_members_for_household", base_address.clone()).await;
