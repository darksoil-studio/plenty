...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.flattened::<EntryTypes, LinkTypes>()? {
                ...
                OpUpdate::Entry { app_entry, action } => {
                    ...
                    match app_entry {
                        EntryTypes::Producer(producer) => {
                            ...
                            let original_producer = match Producer::try_from(
                                original_app_entry,
                            ) {
                                Ok(entry) => entry,
                                Err(e) => {
                                    return Ok(
                                        ValidateCallbackResult::Invalid(
                                            format!("Expected to get Producer from Record: {e:?}"),
                                        ),
                                    );
                                }
                            };
        ...
        FlatOp::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::LiasonToProducers => {
                    validate_create_link_liason_to_producers(
                        action,
                        base_address,
                        target_address,
                        tag,
                ...
                LinkTypes::ProducerUpdates => {
                    validate_create_link_producer_updates(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        FlatOp::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::LiasonToProducers => {
                    validate_delete_link_liason_to_producers(
                        action,
                        original_action,
                        base_address,
                        target_address,
