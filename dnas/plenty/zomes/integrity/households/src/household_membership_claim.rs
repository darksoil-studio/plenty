...
pub fn validate_create_household_membership_claim(
    ...
    let member_create_link_action =
        must_get_valid_record(household_membership_claim.member_create_link_hash)?
            .signed_action
            .hashed;
    let Action::CreateLink(create_link) = member_create_link_action.content else {
        return Ok(ValidateCallbackResult::Invalid(String::from(
            "member_create_link_hash must point to a CreateLink action",
        )));
    };
...
pub fn validate_update_household_membership_claim(
    ...
    Ok(ValidateCallbackResult::Invalid(String::from(
        "Household Membership Claims cannot be updated",
    )))
}
